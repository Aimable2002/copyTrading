...
# Performance-fee rate feature disabled - platform is subscription-only
# now. Kept commented rather than removed in case this is revisited.
...
# Performance-fee billing disabled - the platform monetizes via follower
# subscription packages only now, not a per-master profit share.
# process_follower_deals() and run_poll_cycle() below are kept commented
# rather than deleted in case this is revisited; run_poll_cycle() was
# already not wired into main.py's scheduler prior to this change, so
# nothing was actually calling this path at runtime.
#
...
def get_master_earnings(master_account_id: str, supabase_client: Any, limit: int = 100) -> dict:
    """Earnings are challenge_reward only now - the platform doesn't take
    a profit-share cut of follower profits any more, so a master's income
    is purely what they've earned from challenge rewards. The
    profit_share_master query is commented out below rather than removed:
    any historical rows stay in wallet_transactions untouched, this
    function just stops folding them into "earnings" going forward."""
