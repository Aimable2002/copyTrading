...
from ..copydesk_fanout.order_pair_store import OrderPairStore
...
def _seed_copied_ticket(db, *, follower_account_id: str, follower_ticket: str, master_account_id: str = "master_A", master_ticket: str = "m1") -> None:
    """Marks a ticket as a confirmed copy in order_pairs - what
    OrderPairStore.was_follower_ticket_copied() checks. Without a matching
    row here, process_follower_deals() now correctly refuses to bill it -
    see test_profit_share_skips_deals_never_confirmed_as_a_copy below for
    the test that specifically proves that refusal."""
    db.table("order_pairs").insert(
        {
...
def test_profit_share_bills_wins_skips_losses_and_is_idempotent():
    ...
    pair_store = OrderPairStore(supabase_client=db)
    _seed_copied_ticket(db, follower_account_id="follower_1", follower_ticket="1002")

    charges = profit_share.process_follower_deals(follower_account_id="follower_1", billing_period_id=period_id, agent=agent, supabase_client=db, pair_store=pair_store)
    ...
    charges_again = profit_share.process_follower_deals(follower_account_id="follower_1", billing_period_id=period_id, agent=agent, supabase_client=db, pair_store=pair_store)
...
def test_profit_share_can_push_wallet_negative_master_still_gets_full_cut():
    ...
    pair_store = OrderPairStore(supabase_client=db)
    _seed_copied_ticket(db, follower_account_id="follower_1", follower_ticket="2001")
    charges = profit_share.process_follower_deals(follower_account_id="follower_1", billing_period_id=period_id, agent=agent, supabase_client=db, pair_store=pair_store)
...
def test_profit_share_skips_deals_never_confirmed_as_a_copy():
    """The actual gap this fix closes: a profitable, closed deal that was
    never a confirmed copy of any master signal - e.g. the follower's own
    manual trade on the same MT5 account - must NOT be billed just because
    it happened to close in profit. Same setup as the wins/losses test
    above, except deliberately NOT seeding order_pairs for this ticket."""
    db = _fresh_db()
    wallet.top_up("follower_1", 100.0, db)
    master_rate.set_rate("master_A", 25.0, 10.0, db)
    period_id = _seed_billing_period(db)
    ...
    # Profitable, closed, real deal - but never inserted into order_pairs,
    # i.e. never actually dispatched/confirmed as a copy of anything.
    agent = _FakeAgent({"9001": {"entry": "entry_out", "pnl": 500.0, "symbol": "BTCUSD", "type": "sell", "lots": 0.1, "magic": 0, "deal_time": "", "deal_price": 0, "commission": 0, "swap": 0, "comment": ""}})
    pair_store = OrderPairStore(supabase_client=db)

    charges = profit_share.process_follower_deals(follower_account_id="follower_1", billing_period_id=period_id, agent=agent, supabase_client=db, pair_store=pair_store)
    assert charges == []  # must NOT be billed - not a confirmed copy
    assert wallet.get_wallet("follower_1", db)["balance"] == 100.00  # untouched
    print("PASS test_profit_share_skips_deals_never_confirmed_as_a_copy")
